use crate::skills::{RuleResult, SkillDefinition, ValidationResult};
use regex::Regex;
...
fn evaluate_rule(
    skill: &SkillDefinition,
    rule: &crate::skills::ValidationRule,
    params: &serde_json::Map<String, serde_json::Value>,
) -> RuleResult {
    let value = params.get(&rule.field);

    match rule.rule_type {
        crate::skills::ValidationRuleType::Assert => {
        ...
        crate::skills::ValidationRuleType::Regex => {
            let passed = match value {
                Some(serde_json::Value::String(s)) => match &rule.pattern {
        ...
        crate::skills::ValidationRuleType::Enum => {
            let passed = match value {
                Some(serde_json::Value::String(s)) => match &rule.values {
        ...
        crate::skills::ValidationRuleType::Custom => {
...
fn evaluate_custom_rule(
    ...
    match rule.name.as_str() {
        "chart_type_for_pie" => match params.get("chart_type").and_then(|v| v.as_str()) {
            Some("pie") => match params.get("data").and_then(|v| v.as_array()) {
                Some(data) => data
                    .first()
        ...
        "sections_have_heading" | "sections_have_content" => match params.get("sections") {
            Some(serde_json::Value::Array(sections)) => {
                for section in sections {
                    match section {
                        serde_json::Value::Object(map) => {
                            if rule.name == "sections_have_heading" && !map.contains_key("heading")
                            {
                                return false;
                            }
                            if rule.name == "sections_have_content" && !map.contains_key("content")
                            {
                                return false;
                            }
                        }
        ...
        "slides_have_title" | "slides_have_type" => match params.get("slides") {
        ...
        "line_items_valid" => match params.get("line_items") {
        ...
        "line_items_positive" => match params.get("line_items") {
        ...
        "tax_rate_range" => match params.get("tax_rate") {
        ...
        "markdown_has_content" => match params.get("markdown_text") {
            Some(serde_json::Value::String(s)) => s.contains(|c: char| c == '#'),
            _ => false,
        },
        "data_consistent_cols" => match params.get("data") {
        ...
        "data_min_cols" => match params.get("data") {
            Some(serde_json::Value::Array(data)) => data
                .first()
                .and_then(|row| row.as_array())
                .map(|r| r.len() >= 2)
