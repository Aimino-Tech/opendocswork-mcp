...
impl Xref {
    ...
    pub fn merge(&mut self, xref: Xref) {
        for (id, entry) in xref.entries {
            // Use insert (not or_insert) so PREV entries can override dummy entries
            // from linearized PDFs where the current xref has FREE entries
            // for objects that are actually used.
            if let Some(existing) = self.entries.get(&id) {
                match existing {
                    XrefEntry::Free => {
                        // Override free entries with actual data from PREV xref
                        self.entries.insert(id, entry);
                    }
                    ...
                    _ => {
                        // Keep existing entry for normal entries (PREV is older)
                    }
                }
            } else {
                self.entries.insert(id, entry);
