            ...
            if let XrefEntry::Normal { offset, .. } = *entry {
                ...
                if let Ok(ref mut stream) = object.as_stream_mut() {
                    if stream.dict.has_type(b"ObjStm") {
                        // For encrypted PDFs, ObjStm content is still RC4/AES
                        // encrypted at this point — decoding must wait until after
                        // doc.decrypt().  The application calls
                        // resolve_object_streams() after decryption to handle this.
                        if !self.document.trailer.has(b"Encrypt") {
